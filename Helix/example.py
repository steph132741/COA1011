"""
clinical_data_processor.py (fixed)

Changes made:
- Removed "Use Test Mode (local files)" completely.
- Progress bar changed to determinate row-by-row updates (0 -> 100%).
- Test-mode branches removed so code always uses real FTP processor.
- Minimal changes only: plumbing for progress updates.
"""
...
class ClinicalDataValidator:
    ...
    def _validate_csv_content(self, file_path, status_queue=None, progress_callback=None):
        ...
        Also reports progress via progress_callback(percent) or via status_queue with (percent, "progress")
        """
        errors = []
        valid_records = []
        seen_records = set()
        ...
        try:
            ...
            if isinstance(file_path, (str, Path)):
                with open(file_path, 'r', newline='', encoding='utf-8') as fobj:
                    ...
                    # Load remaining rows into memory to get total (for accurate progress)
                    rows = list(reader)
                    total_rows = len(rows)
                    if total_rows == 0:
                        if status_queue:
                    ...
                    # Iterate rows with index for progress
                    for idx, row in enumerate(rows, start=1):
                        ...
                        if len(row) != 9:
                            ...
                            # update progress
                            percent = (idx / total_rows) * 100.0
                            if progress_callback:
                                progress_callback(percent)
                            elif status_queue:
                                status_queue.put((percent, "progress"))
                            continue
                    ...
                    if errors:
                        ...
                        if progress_callback:
                            ...
                            status_queue.put((100.0, "progress"))
    ...
    def validate_selected_files(self, ftp_obj, files, status_queue):
        """Validate specific files without archiving. ftp_obj is ftplib.FTP"""
        valid_count = 0
        invalid_count = 0
    ...
    def process_selected_files(self, ftp_obj, files, status_queue):
        ...
        for filename in files:
            ...
            try:
                ...
                # Validate content with progress updates
                is_valid, errors, record_count = self._validate_csv_content(
...
class ClinicalDataGUI:
    ...
    def __init__(self, root):
        ...
        # TEST MODE REMOVED (self.test_mode removed)
    ...
    def create_widgets(self):
        ...
        self.progress = ttk.Progressbar(right_panel, mode='determinate', style='Modern.Horizontal.TProgressbar', maximum=100.0)
    ...
    def check_queue(self):
        try:
            while True:
                ...
                # Handle progress messages (numeric percent)
                if tag == "progress":
                    try:
                        ...
                        # if the progress message wasn't numeric, log it
                ...
                if message == "complete" and tag == "complete":
                    # stop indicators
                    if hasattr(self, 'progress'):
                        # ensure progress is at 100 if we reached completion
                        try:
                ...
                if tag in ["complete", "error"]:
                    ...
                    self.update_status_label()
                    if hasattr(self, 'progress'):
                        # on completion, ensure progress is 100
                        try:
                            self.progress['value'] = 100.0
                        except Exception:
                            pass
    ...
    def _validate_selected_worker(self, files):
        try:
            ...
            # Provide progress via status_queue by _validate_csv_content() (it emits (percent, "progress"))
